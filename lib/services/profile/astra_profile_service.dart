...
/// Lightweight local identity profile used by the ASTRA shell.
///
/// The nickname is user-controlled and survives auth changes. Google account
/// metadata is advisory and only fills missing identity fields.
class AstraProfile {
...
class AstraProfileService {
  ...
  /// Sync account metadata without overwriting a nickname the user explicitly set.
  Future<AstraProfile> syncGoogleAccount({
    ...
    if (source != 'user' && currentNickname.trim().isEmpty) {
      ...
      if (candidate.isNotEmpty) {
        await prefs.setString(nicknameKey, candidate);
        await prefs.setString(nicknameSourceKey, 'google');
      } else {
        final localPart = email.split('@').first.trim();
        if (localPart.isNotEmpty) {
          await prefs.setString(nicknameKey, localPart);
          await prefs.setString(nicknameSourceKey, 'google');
        }
      }
    }
  ...
  Future<AstraProfile> setGoogleConnected(bool connected) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(googleConnectedKey, connected);
    if (!connected) {
