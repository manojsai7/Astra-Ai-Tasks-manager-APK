...
class AstraCommandExecutor {
  ...
  Future<AstraExecutionResult> execute({
    ...
    switch (command.intent) {
      ...
      case 'CREATE_CALENDAR_EVENT':
        return _createTaskLikeCommand(ref: ref, command: command);
      default:
        throw UnsupportedError('AstraCommandExecutor does not execute intent: ${command.intent}');
    }
  }

  /// Canonical unified execution for [TaskIntent] from UI and conversational flows.
  /// Task persistence keeps the semantic task state; reminder scheduling uses the
  /// resolved next occurrence from AstraScheduleResolver.
  Future<AstraExecutionResult> executeTaskIntent({
    ...
    if (resolved.shouldScheduleReminder && resolved.reminderScheduleInstant != null) {
      ...
      final reminderAt = resolved.reminderScheduleInstant!;
      await db.upsertReminder(
        RemindersCompanion(
          id: Value(reminderId),
          taskId: Value(task.id),
          scheduledAt: Value(reminderAt),
    ...
    return AstraExecutionResult(
      ...
      taskId: task.id,
      title: task.title,
      scheduledAt: resolved.effectiveDueAt,
  ...
  Future<AstraExecutionResult> update({
    ...
    if (command.requiresConfirmation || !command.hasChanges) {
      return AstraExecutionResult(
        ...
        title: '',
        scheduledAt: null,
        message: command.warnings.isNotEmpty ? command.warnings.first : 'Confirmation required before updating.',
        requiresConfirmation: true,
        confirmationReason: command.targetQuery.isEmpty ? 'missing_target' : (command.hasChanges ? 'ambiguous_time' : 'missing_changes'),
        warnings: command.warnings,
      );
    }

    final resolution = taskResolver.resolve(tasks: activeTasks, query: command.targetQuery);
    ...
    switch (resolution.outcome) {
      ...
      case TaskResolutionOutcome.exact:
        ...
        final originalHasDate = RegExp(
          r'\b(today|tomorrow|tmrw|monday|tuesday|wednesday|thursday|friday|saturday|sunday|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b',
          caseSensitive: false,
        ).hasMatch(command.originalText);
        ...
        final newPriority = command.newPriority != null ? _normalizePriority(command.newPriority!) : existing.priority;
        ...
        final resolved = AstraScheduleResolver.resolve(task: updatedTask, now: DateTime.now());
        await ref.read(taskNotifierProvider.notifier).updateTask(updatedTask);
        ref.invalidate(taskListProvider);

        final strategy = ReminderStrategyX.resolve(priority: updatedTask.priority, eventType: updatedTask.category);
        if (resolved.shouldScheduleReminder && resolved.reminderScheduleInstant != null) {
          await ref.read(reminderServiceProvider).scheduleReminder(
  ...
  Future<AstraExecutionResult> _createTaskLikeCommand({
        ...
        for (final t in dbTasks) {
          ...
          if (tTitle == qTitle || tTitle.contains(qTitle) || qTitle.contains(tTitle)) {
            existingTask = Task(
              ...
              status: t.status,
              recurrenceRule: t.recurrenceRuleJson != null ? RecurrenceRule.fromJson(t.recurrenceRuleJson!) : null,
            );
            break;
          }
        }
    ...
    DateTime? semanticDueDate = command.temporal.eventStart ?? command.temporal.deadline;
    String? semanticDueTime;
    if (command.temporal.rawTime != null) {
      final parsed = _parseRawHourMinute(command.temporal.rawTime!);
      if (parsed != null) {
        semanticDueTime = '${parsed.hour.toString().padLeft(2, '0')}:${parsed.minute.toString().padLeft(2, '0')}';
      }
    }
    ...
    final seedTask = Task(
      id: taskId,
      title: taskTitle,
      dueDate: semanticDueDate,
      dueTime: semanticDueTime,
      startAt: command.temporal.eventStart,
      endAt: command.temporal.eventEnd,
      status: semanticDueDate != null || semanticDueTime != null || recurrenceRule != null ? 'active' : 'pending',
      priority: _normalizePriority(command.priority),
      createdAt: now,
      source: 'assistant',
    ...
    final resolved = AstraScheduleResolver.resolve(task: seedTask, now: now);
    final scheduledAt = resolved.effectiveDueAt;
    ...
    return AstraExecutionResult(
      ...
      message: _successMessage(command, scheduledAt),
      calendarSynced: calendarSynced,
  ...
  String _buildCalendarDescription(AstraCommand command) {
    ...
    if (command.action != null && command.action!.isNotEmpty) buffer.writeln('Action: ${command.action}');
    if (command.eventType.isNotEmpty && command.eventType != 'OTHER') buffer.writeln('Type: ${command.eventType}');
    if (command.organization != null && command.organization!.isNotEmpty) buffer.writeln('Organization: ${command.organization}');
    if (command.originalText.isNotEmpty) buffer.writeln('Original: ${command.originalText}');
    return buffer.toString().trim();
  }

  RecurrenceRule? _buildRecurrenceRule(AstraCommand command) {
    final freqStr = command.recurrence.toUpperCase().trim();
    if (freqStr == 'NONE' || freqStr.isEmpty) return null;
    ...
    if (freq == RecurrenceFrequency.weekdays) {
      byWeekdays.addAll([DateTime.monday, DateTime.tuesday, DateTime.wednesday, DateTime.thursday, DateTime.friday]);
  ...
  DateTime? _calculateFirstOccurrence(
    ...
    final resolved = AstraScheduleResolver.resolve(
      ...
      startAt: command.temporal.eventStart,
      endAt: command.temporal.eventEnd,
      dueTime: command.temporal.rawTime == null ? null : _formatRawTime(command.temporal.rawTime!),
      recurrenceRule: rule,
      now: now,
    );
  ...
  String? _formatRawTime(String raw) {
    final parsed = _parseRawHourMinute(raw);
    if (parsed == null) return null;
    return '${parsed.hour.toString().padLeft(2, '0')}:${parsed.minute.toString().padLeft(2, '0')}';
  }

  _TimeHourMinute? _parseRawHourMinute(String raw) {
    final match = RegExp(r'(\d{1,2})(?::(\d{2}))?\s*(am|pm)?', caseSensitive: false).firstMatch(raw);
  ...
  String _buildDescription(AstraCommand command) {
    final parts = <String>['Created via ASTRA Assistant.'];
    if (command.action != null && command.action!.trim().isNotEmpty) parts.add('Action: ${command.action}');
    if (command.eventType.trim().isNotEmpty && command.eventType != 'OTHER') parts.add('Type: ${command.eventType}');
    if (command.recurrence != 'NONE') parts.add('Recurrence: ${command.recurrence}');
    if (command.temporal.rawDate != null && command.temporal.rawDate!.trim().isNotEmpty) parts.add('Date: ${command.temporal.rawDate}');
    if (command.temporal.rawTime != null && command.temporal.rawTime!.trim().isNotEmpty) parts.add('Time: ${command.temporal.rawTime}');
    if (command.temporal.rawDeadline != null && command.temporal.rawDeadline!.trim().isNotEmpty) parts.add('Deadline: ${command.temporal.rawDeadline}');
    return parts.join('\n');
  }
  ...
  String _successMessage(AstraCommand command, DateTime? scheduledAt) {
    if (scheduledAt == null) return '${command.title} created.';
    return '${command.title} scheduled for ${scheduledAt.toLocal()}.';
  }
}
...
class _TimeHourMinute {
  final int hour;
  final int minute;
  const _TimeHourMinute({required this.hour, required this.minute});
}
